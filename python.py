...
class QuizApp(tk.Tk):
    def __init__(self):
        ...
        def set_background(frame, image):
            bg_label = tk.Label(frame, image=image)
            bg_label.pack(fill='both', expand=True)
            bg_label.image = image

        # Home frame setup
        self.home_frame = tk.Frame(self)
        set_background(self.home_frame, self.bg_image)
        ...
        set_background(self.name_frame, self.name_bg_image)
        ...
        set_background(self.leaderboard_frame, self.leaderboard_bg_image)  # Set leaderboard background

        # Fun fact frame (hidden by default)
        self.funfact_frame = ctk.CTkFrame(self)
        self.funfact_frame.pack_forget()
        set_background(self.funfact_frame, self.funfact_bg_image)  # Set funfact background
    ...
    def initialize_quiz_ui(self):
        ...
        buttonfont3 = ctk.CTkFont(family="Helvetica", size=16)  # Font for submit and back buttons
        radio_font = ctk.CTkFont(family="Helvetica", size=18)  # Font for radio buttons
        ...
        self.submit_button = ctk.CTkButton(self.quiz_frame, text='Submit Answer', command=self.check_answer,
                                           font=buttonfont3)
        self.submit_button.place(relx=0.75, rely=0.6, anchor=tk.CENTER)  # Place at the bottom right
